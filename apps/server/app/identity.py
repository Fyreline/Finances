"""The Mishka Hub identity client — docs/AUTH.md.

Kakeibo holds no credential store of its own. At login it forwards the
...
``refresh_tokens`` table stays tidy). Port of Michi's ``identity.py``
(docs/AUTH.md — "Michi's implementation is the port source").
"""
...
class MishkaIdentityClient:
    def __init__(self, base_url: str, timeout: float = 5.0) -> None:
        ...
        if parsed.scheme == "http" and not is_loopback:
            raise ValueError(
                "KAKEIBO_MISHKA_BASE_URL must be HTTPS or loopback "
                f"(got {base_url!r}) — docs/AUTH.md §2"
            )
    ...
    async def verify(self, email: str, password: str) -> IdentityUser:
        ...
        fixed message, never the request body (docs/AUTH.md).
        """
        try:
    ...
    async def _best_effort_logout(self, client: httpx.AsyncClient, refresh_token: str) -> None:
        """Keeps Mishka's refresh_tokens table tidy (docs/AUTH.md). Never
        raises — a failure here must never fail the Kakeibo login."""
        try:
