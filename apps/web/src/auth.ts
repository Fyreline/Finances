...
// apps/web/src/auth.ts: base URL defaults to Kakeibo's own dev server
// (127.0.0.1:8201) and the localStorage key is `kakeibo-refresh-token` — a
// distinct key from `mishka-refresh-token` and `michi-refresh-token`, since
// all three SPAs may share `localhost` during dev and clobbering a
// sibling's session would be a rude bug (docs/AUTH.md §2 "Kakeibo-specific
// deltas").
//
...
const REFRESH_STORAGE_KEY = 'kakeibo-refresh-token'
