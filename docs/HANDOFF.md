...
| **Phase 14 — income-anchor staleness, manual weekday payday, recurring glance clarity** | 🔜 **Spec written 2026-07-12** (`docs/phases/PHASE-14-income-anchor-and-recurring-glance.md`), not yet implemented. Real feedback after a real job change: safe-to-spend's detected pay period went nonsensical, and `remaining_minor`/`per_day_remaining_minor` went deeply negative while the headline stayed positive — both root-caused directly to the same bug: `_detect_income_anchor` has no staleness check, so it keeps confidently selecting a former employer's long-dead-but-historically-high-confidence pattern over a genuinely fresh one, and the new employer's fresh cluster doesn't even clear `_MIN_OCCURRENCES` yet because its first (likely prorated) payment doesn't amount-cluster with the following ones. Fix needs three parts: (1a) a staleness check so a gone-quiet pattern stops being treated as current; (1b) a bounded amount-outlier tolerance (mirroring Phase 13's gap-outlier fix, same spirit) so a fresh cluster with one odd first payment can still register; (1c) a genuine manual weekday-payday override (`payday_weekday`/`payday_week_position`, new nullable `financial_config` columns) since detection will always have a cold-start gap after a job change no matter how good the algorithm gets — the user has asked for this twice. Also item 2: the Recurring bubble's glance shows no amount for its "next" line, and surfaces confusing raw-counterparty labels for what are really internal transfers, not bills — add the amount, prefer an active pattern over a lapsed one for the headline slot, and make the existing "not a subscription" dismiss affordance easier to discover. Owner: Opus, same tier as Phase 4/5/11/12/13. |
