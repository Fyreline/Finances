...
| **Phase 13 — rental history/deductions/cleanup + the real safe-to-spend bug** | ✅ **complete 2026-07-12** (Opus). Real feedback after live use of Phase 12's output, four items, all root-caused before the spec was written. **(A) multi-year pull + UI:** `gmail_search_days` widened 400 → 1700 (~4.6y, comfortably generous rather than derived from a real date, per PRIVATE.md's redaction scheme) — dedup on `gmail_message_id` makes a wide re-run harmless, so this is now the standing default, not a one-off. Ran the real historical pull (direct script invocation, real prod DB/Gmail, never a forged session): document folders now exist for several additional tax years beyond the previous 2, and the confirmed rent-statement/ledger pipeline (Phase 12) ran against them unchanged. New `GET /api/tax/years` (`{years, current_tax_year}`, filters out years that only ever got unrelated 'other' mail so the selector isn't cluttered) — `TaxDetail.tsx` now has a real year selector wired through Documents/Ledger/Estimate, replacing the old hardcoded-to-current-year constant. **(B) delete confirmed-non-rental docs:** new `app/rental_document_cleanup.py` (`delete_non_rental_documents`) + `scripts/delete_non_rental_documents.py` — deletes `doc_type='other'` rows and their on-disk folders (skips, never deletes, any document a human has since manually linked into the ledger); `insurance`/`mortgage_interest_cert` are structurally excluded from the deletable set, never touched. Backed up the prod DB and the full `tax-documents/` tree first (`data/backups/kakeibo-pre-phase13-*.db`, `data/backups/tax-documents-pre-phase13*`); ran against the real prod data — every `other`-typed document was removed from both DB and disk, `insurance` rows (and everything else) untouched, verified directly by doc_type count before/after. **(C) itemised deductions:** `engines/rent_statement_parser.py` now parses the "Property Costs Summary for Month" section (0..n free-text `<description> £X.XX` lines) into one combined `repairs` ledger row, cross-checked against the statement's own "Total Costs" figure (a reconciliation mismatch leaves the deduction unledgered for review rather than guessing which figure is right) — falls back to the legacy single landlord-direct line for statements with no costs section. `rent_statement_ingest.py` gained an additive top-up path (`TOPPED_UP` outcome) so a document Phase 12 already ledgered without a repairs row gets one added on the next backfill run, without touching or duplicating its existing income/agent_fees rows. Re-ran the backfill against the real prod data: previously-missed deductions (including the real June statement's) now have matching `repairs` ledger rows, verified directly against the ledger (never restated here). **(D) — the real safe-to-spend bug, fixed and verified live:** `engines/recurring.py::cadence_for_gaps` — widened the monthly cadence window 28–33 → 27–36 days (a weekday-anchored payday's natural 28-vs-35-day alternation), and replaced the unconditional "any gap > 1.6× median vetoes the whole cluster" rule with a bounded allowance (`len(gaps) // 8`, floored — a short cluster still tolerates zero outliers, preserving the original protection against false-positiving a genuinely irregular transfer) that sets aside a small number of holiday-shifted/delayed beats and classifies on the median of what remains. Regression-tested both directions: a synthetic reproduction of the real last-Friday-payday-with-a-Christmas-gap shape is now detected monthly (25+ occurrences survive), and all 8 already-working synthetic outgoing committed-cost patterns (monthly/weekly/annual) are unaffected. **Verified directly against the real prod DB** (script invocation, never a forged session): `safe_to_spend_payload` now returns `payday_source`/`net_income_source` = `'detected'`, `setup_missing: []`, and a real non-null `safe_to_spend_minor` — the actual acceptance bar, not just green tests. 385 server tests (367 inherited + 18 new), 41 web tests, typecheck/build clean. **This phase touched `apps/server/` → the `com.kakeibo.api` LaunchAgent needs a manual kickstart after commit (orchestrator, not this phase).** **Pre-existing redaction note (not introduced this phase):** the already-committed PHASE-13 spec doc's item-B section quotes the user's own words verbatim, which names the real letting agent — flagged for the orchestrator's attention, left as-is since this phase didn't touch that file; a fresh restatement of the same quote was caught and genericised in this phase's own new `rental_document_cleanup.py` before commit. |
| **Real savings-deals research run, 2026-07-12** | ✅ **First real (non-synthetic) research pass, done directly by the orchestrator per `scripts/research_deals_prompt.md` — no code change.** WebSearched MoneySavingExpert's best-buy table plus independently cross-checked 4 of the 5 providers' own pages (Revolut, Cahoot, Chase, Trading 212) for the current rate/terms; `data/deals/2026-07-12.json` written and imported directly into the real prod DB via `deals_service.import_newest_deal_run` (no server restart needed — deals are read fresh from the DB on every request, not cached at startup). 5 real easy-access deals now live on the Deals bubble, each with its real source URL and honest notes about bonus/reversion terms. |
| **Phase 14 — income-anchor staleness, manual weekday payday, recurring glance clarity** | ✅ **complete 2026-07-12** (Opus). Real feedback after a job change, all four items done. **(1a) staleness-aware anchor selection** (`insights_service._detect_income_anchor`): a detected monthly income pattern whose most recent payment is more than `_ANCHOR_STALENESS_FACTOR` (1.8) × its own observed median cadence ago is dropped — a gone-quiet former pattern stops being confidently selected as the current salary the moment it goes stale, however historically high-confidence it was; if that disqualifies every candidate the detection-reliant fields fall to `setup_missing` (honest "no confident answer"), never a stale-rolled-forward period. A manually-set `net_monthly_income_minor` is untouched by this. **(1b) earliest-amount-outlier tolerance** (`recurring.cluster_by_amount(..., tolerate_earliest_outlier=True)`, forwarded through `detect_recurring`, on only for the `direction="in"` anchor path): a new job's prorated first paycheck (a lone leading amount-outlier) is folded into the otherwise-tight steady run behind it so the run can still reach `_MIN_OCCURRENCES=3`; narrowly scoped (single earliest txn, only when ≥2 later occurrences already agree tightly), off by default so the outgoing committed-cost path is provably unchanged. **(1c) manual weekday payday override**: new nullable `financial_config.payday_weekday` (0=Mon..6=Sun) + `payday_week_position` ('first'|'second'|'third'|'fourth'|'last') columns (self-heal via `schema_sync.py`), a pure `insights.payday_period_from_weekday()`/`nth_weekday_of_month()` mirroring `payday_period`'s boundary semantics, wired into `resolve_period`'s manual branch (numeric `payday_day` keeps precedence; the PUT keeps the two kinds mutually exclusive by clearing the other pair); web `ConfigForm` gained a day-of-month ↔ day-of-week toggle. Rental-income summing confirmed unaffected by an explicit regression test. **(item 2) recurring glance**: `RecurringGlance` now shows the amount on the "next" line (how much, for what, when) and prefers an `active` pattern over a `lapsed` one with an earlier `next_expected` (new pure `pickNextRecurring`); `RecurringDetail` gained a discoverability line pointing at the existing "not a subscription" dismiss for internal-transfer noise. **Verified read-only against a COPY of the real prod DB** (schema_sync'd like boot, real prod DB untouched, no forged session): 3 incoming monthly candidates — one 26-occurrence high-confidence pattern correctly flagged STALE and dropped (1a), one 3-occurrence fresh pattern that only forms because of 1b's fold (0 fresh monthly without the tolerance), now selected as the anchor → `payday_source='detected'`, `setup_missing=[]`, a sane 32-day current period (16 days in) replacing the nonsensical stale-rolled-forward one; manual net income preserved. 402 server tests (385 inherited + 17 new), 45 web tests (41 + 4 new), typecheck/build clean; full redaction grep sweep clean. Owner: Opus. **This phase touched `apps/server/` → the `com.kakeibo.api` LaunchAgent needs a manual kickstart after commit (orchestrator, not this phase) — the new nullable columns also self-heal on that boot.** |
